...
locals {
  ...
  route_entries = {
    ...
    "b_untrust-to-a"        = { subnet = "b_untrust", destination = "10.0.0.0/16", igw = null, tgw = "tgw1" }
    "b_untrust-to-c"        = { subnet = "b_untrust", destination = "10.2.0.0/16", igw = null, tgw = "tgw1" }
    "b_untrust-to-d"        = { subnet = "b_untrust", destination = "10.3.0.0/16", igw = null, tgw = "tgw2" }
...
# ALB Security Group (created here to avoid circular dependency with security module)
resource "aws_security_group" "alb" {
  description = "Internet-facing ALB security group in VPC-B."
  name        = "lab-sg-alb"
  ...
  egress {
    description = "HTTPS to Palo untrust ENI"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
